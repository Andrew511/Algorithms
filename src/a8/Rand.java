package a8;/* Rand.java - Randomization code
 *
...
class Rand {
    ...
    static void shuffleArray(int[] a, int seed, int n) {
        ...
        // check if n is larger that the length of a, if so, just fill with 1s
        if (n > a.length - 1) {
            for (int i = 0; i < doSwap.length; i++) {
                doSwap[i] = 1;
            }
        } else {
            int numOnes = n;
        ...
        //2.
        if (n < a.length - 1) {
            fisherYates(doSwap, seed);
            if (doSwap[0] == 1) {
                for (int i = 1; i < doSwap.length; i++) {
                    if (doSwap[i] == 2) {
                        swap(doSwap, 0, i);
                        break;
                    }
